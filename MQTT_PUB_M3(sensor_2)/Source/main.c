...
void main(void)
{
	...
	mqtt_init(&broker, "sensor2_device");		//	client ID  (다수일때 다르게 해줘야 함)
	...
	for(;;) // 수정할 부분
	{
                ...
                if(Key_Value)       //변수 추가
                {
                      if(sensor[Key_Value] == 0)        // sensor off 일 때
                      {
                        ...
                        sprintf(message_sensor,"!%d%d",2,Key_Value);    // 구역 1 
                        mqtt_publish(&broker,"sensor1",message_sensor,0);
                      }
                      else      // sensor on 일 때
                      {
                        Uart1_Printf("KEY=%d ON\n", Key_Value);
                        sprintf(message_sensor,"@%d%d",2,Key_Value);        // 구역 1 (구역 번호)프로토콜 추가함 
                        mqtt_publish(&broker,"sensor1",message_sensor,0);
                      }
