...
int mqtt_connect(mqtt_broker_handle_t* broker)
{
	...
	uint8_t packet[100];
//	uint8_t packet[100] = {0x10, 0x25, 0x00, 0x06, 0x4d, 0x51,0x49,0x73,0x64,0x70,
//		3, 2, 0, 0x3c, 0, 0x17, 0x6d, 0x6f, 0x73, 0x71, 0x70, 0x75, 0x62, 0x2f, 0x33, 0x34,
//		0x31, 0x36, 0x2d, 0x4d, 0x53, 0x44, 0x4e, 0x2d, 0x53, 0x50, 0x45, 0x43, 0x49};
	...
	// Send the packet
//	if(broker->send(broker->socket_info, packet, 39) < 39) {
//		return -1;
//	}
//	return 1;
	...
	// Send the packet
	if(broker->send(broker->socket_info, packet, remainLen+fixedHeaderSize) < remainLen + fixedHeaderSize) {
		return -1;
	}
...
int mqtt_publish_with_qos(mqtt_broker_handle_t* broker, const char* topic, const char* msg, uint8_t retain, uint8_t qos, uint16_t* message_id) {
	...
	uint8_t var_header[80]; // Topic size
	uint8_t fixedHeaderSize = 2;    // Default size = one byte Message Type + one byte Remaining Length
	uint16_t remainLen;
	uint8_t fixed_header[3];
	uint8_t packet[100];
	...
	remainLen = topiclen + 2 + qos_size + msglen;
	// Fixed header
   ...
   if (remainLen <= 127) {
	...
	memcpy(packet, fixed_header, fixedHeaderSize);
	offset += fixedHeaderSize;
	memcpy(packet+offset, var_header, topiclen + 2 + qos_size);
	offset += topiclen + 2 + qos_size;
...
int mqtt_subscribe(mqtt_broker_handle_t* broker, const char* topic, uint16_t* message_id) 
{
	uint16_t topiclen = strlen(topic);
	uint16_t offset = 0;
	uint8_t utf_topic[80]; // Topic size (2 bytes), utf-encoded topic, QoS byte
	...
	memcpy(packet+offset, var_header, sizeof(var_header));
	offset += sizeof(var_header);
	memcpy(packet+offset, utf_topic, topiclen+2);
	offset += topiclen+2;

	packet[offset] = req_qos;
	offset += sizeof(req_qos);
